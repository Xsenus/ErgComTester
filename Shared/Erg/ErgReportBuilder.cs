...
public static class ErgReportBuilder
{
    ...
    private static void BuildPatientWordReportClassic(ErgPatient patient, string docxPath, CommonInfo? deviceInfo, string? clinicName, string? rawFilePath)
    {
        ...
        using (var document = WordprocessingDocument.Create(docxPath, WordprocessingDocumentType.Document))
        {
            ...
            ApplyWordPackageProperties(document, deviceInfo);
        }
    ...
    private static void ApplyWordPackageProperties(WordprocessingDocument document, CommonInfo? deviceInfo)
    {
        if (document == null)
            return;
        ...
        props.Creator = "Microlux ErgConnect";
        props.Title = title;
        props.Subject = "Electroretinography";
        props.Description = "Автоматически сформированный отчет ЭРГ";
        props.LastModifiedBy = "Microlux ErgConnect";
        props.Created = now;
        props.Modified = now;
    }
    ...
    private static void NormalizeWordPackage(string docxPath)
    {
        ...
        try
        {
            var entries = ReadPackageEntries(docxPath);
            if (!entries.ContainsKey("word/document.xml"))
                return;

            EnsureDocumentPropertyParts(entries);

            var packageRels = BuildPackageRelationshipsDocument(entries);
            entries["_rels/.rels"] = WriteXmlToBytes(packageRels);

            var documentRels = SanitizeDocumentRelationships(entries);
            if (documentRels != null)
            {
                entries["word/_rels/document.xml.rels"] = WriteXmlToBytes(documentRels);
            }
            else
            {
                entries.Remove("word/_rels/document.xml.rels");
            }

            var manifest = BuildContentTypesManifest(entries.Keys);
            var manifestBytes = WriteXmlToBytes(manifest);

            WritePackage(docxPath, entries, manifestBytes);
        }
        catch (Exception ex)
        {
            throw new InvalidOperationException($"Не удалось обновить DOCX-пакет '{docxPath}'.", ex);
        }
    }

    private static Dictionary<string, byte[]> ReadPackageEntries(string docxPath)
    {
    ...
    private static void EnsureDocumentPropertyParts(IDictionary<string, byte[]> entries)
    {
        if (!entries.ContainsKey("docProps/core.xml"))
        {
            entries["docProps/core.xml"] = CreateDefaultCoreProperties();
        }

        if (!entries.ContainsKey("docProps/app.xml"))
        {
            entries["docProps/app.xml"] = CreateDefaultAppProperties();
        }
    }

    private static byte[] CreateDefaultCoreProperties()
    {
        ...
        var root = new XElement(cp + "coreProperties",
            ...
            new XElement(dc + "title", "Отчет ЭРГ"),
        ...
        return WriteXmlToBytes(new XDocument(new XDeclaration("1.0", "UTF-8", "yes"), root));
    }

    private static byte[] CreateDefaultAppProperties()
    {
        var appNs = XNamespace.Get("http://schemas.openxmlformats.org/officeDocument/2006/extended-properties");
        var vtNs = XNamespace.Get("http://schemas.openxmlformats.org/officeDocument/2006/docPropsVTypes");

        var root = new XElement(appNs + "Properties",
            new XAttribute(XNamespace.Xmlns + "vt", vtNs),
            new XElement(appNs + "Application", "Microlux ErgConnect"),
            new XElement(appNs + "DocSecurity", 0),
            new XElement(appNs + "ScaleCrop", false),
            new XElement(appNs + "HeadingPairs",
                new XElement(vtNs + "vector",
                    new XAttribute("size", "2"),
                    new XAttribute("baseType", "variant"),
                    new XElement(vtNs + "variant", new XElement(vtNs + "lpstr", "Paragraphs")),
                    new XElement(vtNs + "variant", new XElement(vtNs + "i4", "1"))
                )
            ),
            new XElement(appNs + "TitlesOfParts",
                new XElement(vtNs + "vector",
                    new XAttribute("size", "1"),
                    new XAttribute("baseType", "lpstr"),
                    new XElement(vtNs + "lpstr", "Document")
                )
            ),
            ...
            new XElement(appNs + "AppVersion", "16.0000")
        );

        return WriteXmlToBytes(new XDocument(new XDeclaration("1.0", "UTF-8", "yes"), root));
    }

    private static XDocument BuildPackageRelationshipsDocument(IReadOnlyDictionary<string, byte[]> entries)
    {
        var ns = XNamespace.Get("http://schemas.openxmlformats.org/package/2006/relationships");
        var root = new XElement(ns + "Relationships");

        int counter = 1;
        string NextId() => $"rId{counter++}";
    ...
    private static XDocument? SanitizeDocumentRelationships(IReadOnlyDictionary<string, byte[]> entries)
    {
        if (!entries.TryGetValue("word/_rels/document.xml.rels", out var data))
            return null;

        var document = TryLoadXml(data);
        ...
        return new XDocument(new XDeclaration("1.0", "UTF-8", "yes"), document.Root);
    }

    private static XDocument BuildContentTypesManifest(IEnumerable<string> entryNames)
    {
        ...
        foreach (var extension in GetImageExtensionsFromEntries(entryNames))
        {
        ...
        foreach (var entryName in entryNames)
        {
            ...
            if (!overrideMap.TryGetValue(normalized, out var contentType))
                continue;
    ...
    private static IEnumerable<string> GetImageExtensionsFromEntries(IEnumerable<string> entryNames)
    {
        return entryNames
            .Where(name => name.StartsWith("word/media/", StringComparison.OrdinalIgnoreCase))
    ...
    private static byte[] WriteXmlToBytes(XDocument document)
    {
        using var memory = new MemoryStream();
    ...
    private static XDocument? TryLoadXml(byte[] data)
    {
        try
        {
            using var memory = new MemoryStream(data);
            return XDocument.Load(memory);
        }
        catch
        {
            return null;
    ...
    private static void WritePackage(string docxPath, IDictionary<string, byte[]> entries, byte[] manifest)
    {
        using var stream = new FileStream(docxPath, FileMode.Create, FileAccess.ReadWrite, FileShare.None);
        using var archive = new ZipArchive(stream, ZipArchiveMode.Create);

        var manifestEntry = archive.CreateEntry("[Content_Types].xml", CompressionLevel.Optimal);
        using (var manifestStream = manifestEntry.Open())
        {
            manifestStream.Write(manifest, 0, manifest.Length);
        }
