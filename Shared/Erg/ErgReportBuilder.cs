...
public static class ErgReportBuilder
{
    ...
    private static SectionProperties ApplyPageMargins(Body body, double leftCm, double rightCm, double? topCm, double? bottomCm)
    {
        if (body == null)
    ...
    private static void AttachFooter(MainDocumentPart mainPart, SectionProperties sectionProps, string? versionText)
    {
        if (mainPart == null || sectionProps == null)
            return;

        var normalizedVersion = string.IsNullOrWhiteSpace(versionText) || versionText == "—"
            ? string.Empty
            : $"Версия отчета: {versionText}";
        ...
        static Run CreateFooterRun(string text, bool preserveSpace = false)
        {
            var runProps = new RunProperties(
                new FontSize { Val = PointsToHalfPointString(8) },
                new WordColor { Val = "777777" }
            );
        ...
        static Run CreateFooterFieldRun(FieldChar fieldChar)
        {
            var runProps = new RunProperties(
                new FontSize { Val = PointsToHalfPointString(8) },
                new WordColor { Val = "777777" }
        ...
        static Run CreateFooterFieldCode(string instruction)
        {
            var runProps = new RunProperties(
                new FontSize { Val = PointsToHalfPointString(8) },
                new WordColor { Val = "777777" }
            ...
            var run = new Run(runProps);
            run.Append(new FieldCode(instruction));
            return run;
        }

        var row = new TableRow();

        var leftCell = new TableCell(new TableCellProperties(new TableCellVerticalAlignment { Val = TableVerticalAlignmentValues.Center }));
        var leftText = string.IsNullOrWhiteSpace(normalizedVersion) ? "\u00A0" : normalizedVersion;
        leftCell.Append(CreateParagraph(leftText, fontSizePt: 8, colorHex: "777777", justification: JustificationValues.Left));
        row.Append(leftCell);

        var rightCell = new TableCell(new TableCellProperties(new TableCellVerticalAlignment { Val = TableVerticalAlignmentValues.Center }));
        var rightParagraph = new Paragraph(new ParagraphProperties(new Justification { Val = JustificationValues.Right }));
        rightParagraph.Append(CreateFooterRun("Стр. "));
        rightParagraph.Append(CreateFooterFieldRun(new FieldChar { FieldCharType = FieldCharValues.Begin }));
        rightParagraph.Append(CreateFooterFieldCode(" PAGE "));
        rightParagraph.Append(CreateFooterFieldRun(new FieldChar { FieldCharType = FieldCharValues.Separate }));
        rightParagraph.Append(CreateFooterRun("1"));
        rightParagraph.Append(CreateFooterFieldRun(new FieldChar { FieldCharType = FieldCharValues.End }));
        rightParagraph.Append(CreateFooterRun(" из "));
        rightParagraph.Append(CreateFooterFieldRun(new FieldChar { FieldCharType = FieldCharValues.Begin }));
        rightParagraph.Append(CreateFooterFieldCode(" NUMPAGES "));
        rightParagraph.Append(CreateFooterFieldRun(new FieldChar { FieldCharType = FieldCharValues.Separate }));
        rightParagraph.Append(CreateFooterRun("1"));
        rightParagraph.Append(CreateFooterFieldRun(new FieldChar { FieldCharType = FieldCharValues.End }));
        ...
        foreach (var existing in sectionProps.Elements<FooterReference>().ToList())
            existing.Remove();
    ...
    private static void BuildPatientReportQuestPdfClassic(ErgPatient patient, string pdfPath, CommonInfo? deviceInfo, string? clinicName, string? rawFilePath)
    {
        ...
        QuestDocument.Create(container =>
        {
            container.Page(page =>
            {
                ...
                {
                    column.Spacing(6);
                    column.Item().Element(header => header.ShowOnce().Column(headerColumn =>
                    {
                        headerColumn.Spacing(0);
                        for (int i = 0; i < clinicHeaderLines.Length; i++)
                        {
                            var line = clinicHeaderLines[i];
                            var padding = i == clinicHeaderLines.Length - 1
                ...
                page.Content().Column(column =>
                {
                    ...
                    for (int i = 0; i < patient.Tests.Count; i++)
                    {
                        ...
                        column.Item().Element(item => item.Component(new TestComponent(i + 1, test)).ShowEntire());
                    }
                ...
                page.Footer().Row(row =>
                {
                    row.RelativeItem().AlignLeft().Text(text =>
                    {
                        ...
                        if (!string.IsNullOrWhiteSpace(reportVersion) && reportVersion != "—")
                            text.Span($"Версия отчета: {reportVersion}");
                        else
                            text.Span(" ");
                    });
        ...
        QuestDocument.Create(container =>
        {
            container.Page(page =>
            {
                ...
                page.Content().Column(column =>
                {
                    ...
                    for (int i = 0; i < patient.Tests.Count; i++)
                    {
                        column.Item().Element(item => item.Component(new ClientTestComponent(i + 1, patient.Tests[i])).ShowEntire());
                    }
    ...
    private static void BuildPatientWordReportClassic(ErgPatient patient, string docxPath, CommonInfo? deviceInfo, string? clinicName, string? rawFilePath)
    {
        using (var document = WordprocessingDocument.Create(docxPath, WordprocessingDocumentType.Document))
        {
            ...
            var sectionProps = ApplyPageMargins(body, leftCm: 2.0, rightCm: 2.0, topCm: 2.0, bottomCm: 2.0);
            AttachFooter(mainPart, sectionProps, version);
    ...
    private static TableCell CreateClientHeaderCell(string text, int gridSpan)
    {
        var props = new TableCellProperties(
            ...
            new Shading { Fill = "D5D5D5", Val = ShadingPatternValues.Clear, Color = "000000" }
        );
        props.Append(new TableCellMargin
        {
            LeftMargin = new LeftMargin { Width = "80", Type = TableWidthUnitValues.Dxa },
    ...
    private static TableCell CreateClientWaveLabelCell(string label)
    {
        ...
        cell.Append(CreateParagraph(text, fontSizePt: 11, justification: JustificationValues.Left));
        return cell;
    }
    ...
    private static Table CreateGraphTable(MainDocumentPart mainPart, GraphImage? rightGraph, GraphImage? leftGraph, ref uint imageId)
    {
        ...
        if (rightGraph == null && leftGraph == null)
        {
            var placeholderCellProps = new TableCellProperties(
                new GridSpan { Val = 2 },
                new TableCellVerticalAlignment { Val = TableVerticalAlignmentValues.Center }
            );
            var placeholderCell = new TableCell(placeholderCellProps);
            placeholderCell.Append(CreateParagraph("Нет данных", fontSizePt: 10, italic: true, colorHex: "666666", justification: JustificationValues.Center));
            table.Append(new TableRow(placeholderCell));
            return table;
        }
    ...
    private static string[] PrepareClinicHeaderLines(string[] lines)
    {
        ...
        var count = Math.Min(lastNonEmpty + 1, 4);
    ...
    private sealed class LegacyPdfRenderer : IDisposable
    {
        ...
        private void DrawGraphSection(ErgTest test, GraphImage? rightGraph, GraphImage? leftGraph)
        {
            ...
            if (rightGraph == null && leftGraph == null)
            {
                var placeholderRect = new RectangleF(_marginLeft, top, ContentWidth, totalHeight);
        ...
        public void Compose(IContainer container)
        {
            container.Border(1).BorderColor(Colors.Grey.Lighten3).Padding(10).Column(column =>
            {
                ...
                column.Item()
                    ...
                    .PaddingBottom(6)
                    .AlignCenter()
                    .Text(FormatClientTestTitle(_index, _test)).FontSize(13).SemiBold();
    ...
    private sealed class ClientTestComponent : IComponent
    {
        ...
        public void Compose(IContainer container)
        {
            ...
            container.Column(column =>
            {
                ...
                if (rightGraph == null && leftGraph == null)
                {
                    column.Item().PaddingVertical(24).AlignCenter().Text(text =>
                    {
                        text.DefaultTextStyle(style => style.FontSize(11).FontColor(Colors.Grey.Darken1));
                        text.Span("Нет данных").Italic();
                    });
                }
                else
                {
                    ...
                    {
                        row.Spacing(18);
                        row.RelativeItem().Component(new ClientGraphComponent(rightGraph));
                        row.RelativeItem().Component(new ClientGraphComponent(leftGraph));
                    });
                }
            });
        }
    ...
    private sealed class ClientGraphComponent : IComponent
    {
        ...
        public ClientGraphComponent(GraphImage? graph) => _graph = graph;
        ...
        public void Compose(IContainer container)
        {
            container.Column(column =>
            {
                ...
                if (_graph != null)
                {
                    column.Item().AlignCenter().Element(e => e.Image(_graph.Data).FitWidth());
                }
                else
                {
                    column.Item().Padding(12)
