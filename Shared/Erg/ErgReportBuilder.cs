...
public static class ErgReportBuilder
{
    ...
    private static void NormalizeWordContentTypes(string docxPath)
    {
        ...
        try
        {
            ...
            XDocument? existingManifest = null;
            ...
            {
                existingManifest = XDocument.Load(reader);
            }

            var entries = archive.Entries.ToList();
            var newManifest = BuildContentTypesManifest(existingManifest, entries);
    ...
    private static XDocument BuildContentTypesManifest(XDocument? existingManifest, IReadOnlyCollection<ZipArchiveEntry> entries)
    {
        ...
        if (existingManifest?.Root != null)
        {
            foreach (var element in existingManifest.Root.Elements(ns + "Override"))
            {
                var partName = NormalizePartName((string?)element.Attribute("PartName"));
                ...
                if (!PackagePartExists(entries, partName))
                    continue;
