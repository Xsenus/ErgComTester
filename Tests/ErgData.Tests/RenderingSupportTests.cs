...
public class RenderingSupportTests
{
    ...
    [Fact]
    public void WordReport_DisplaysActualAmplitudeWhenValueEquals255()
    {
        var outputPath = Path.Combine(Path.GetTempPath(), Guid.NewGuid() + ".docx");
        try
        {
                ...
                Tests =
                {
                    new ErgTest
                    {
                        ...
                        RightEye = new EyeData
                        {
                            QualityIndex = 2,
                            ValueCount = 1,
                            AWaveMs = new ushort?[] { 18 },
                            AWaveMkV = new uint?[] { 255u },
                            BWaveMs = new ushort?[] { 44 },
                            BWaveMkV = new uint?[] { 300u },
                            GraphCount = 0
                        },
                        LeftEye = new EyeData
                        {
                            QualityIndex = 2,
                            ValueCount = 1,
                            AWaveMs = new ushort?[] { 20 },
                            AWaveMkV = new uint?[] { 260u },
                            BWaveMs = new ushort?[] { 50 },
                            BWaveMkV = new uint?[] { 310u },
                            GraphCount = 0
                        }
                    }
                }
    ...
    [Fact]
    public void WordReport_ShowsPlaceholdersWhenOnlyMeasurementSlotsArePresent()
    {
        var outputPath = Path.Combine(Path.GetTempPath(), Guid.NewGuid() + ".docx");
        try
        {
            ...
            {
                PatientId = 103,
                Animal = AnimalKind.Horse,
                TestDateTime = "05.05.2024 12:00",
                TotalNumTests = 1,
                Tests =
                {
                    new ErgTest
                    {
                        ...
                        RightEye = new EyeData
                        {
                            QualityIndex = 2,
                            ValueCount = 0,
                            AWaveMs = new ushort?[] { null },
                            AWaveMkV = new uint?[] { null },
                            BWaveMs = new ushort?[] { null },
                            BWaveMkV = new uint?[] { null },
                            GraphCount = 1,
                            GraphsNormalized = new[]
                            {
                                Enumerable.Repeat(0d, 10).ToArray()
                            }
                        },
                        LeftEye = new EyeData
                        {
